...
TOS is the governed operating system that contains the **control plane and kernel**; it is not the automation standard itself. The TOS control plane observes, diagnoses, plans, and coordinates, while the TOS kernel provides the deterministic privileged-action enforcement boundary. TSAL defines the technology-agnostic rules, contracts, evidence semantics, conformance, and compatibility expectations that TOS consumes. Workloads such as XQueue and Teach retain their own domain behavior and local fail-closed safety controls.
